...
const History = (props) => {
    ...
    async function handleSubmit(e, i) {
        ...
        // const res = await fetch(`https://bloodshare-backend-api.onrender.com/bank/sendDonorCertificate/${e.userId._id}`)
        // const data = await res.json()
        // console.log(data)
        const res = await axios.get(`/bank/sendDonorCertificate/${e.userId._id}`, { withCredentials: true })
        console.log(res)
    }
                        ...
                        data.map((e, i) =>
                            props.user == "bank" ? <tr className={status == "All" ? "" : status != e.status ? "hidden" : ""}>
                                ...
                                <td className='border p-3'>{(() => {
                                    ...
                                    return <>
                                        {date[2]}<br />
                                        <code><small>{date[0].split('T')[0]}</small></code>
                                    </>
                                })()}</td>
                                <td className='border p-3'>
                                ...
                                {props.handle == 'donations' &&
                                    ...
                                    <td className='border p-3'>
                                        <button className='bg-green p-3 rounded-lg text-white-900 ' onClick={() => handleSubmit(e, i)} >Send </button>
                                    </td>
                                }
